...
void odom_reset(Dir senX_dir, Dir Xdir, int Xsen, Dir senY_dir, Dir Ydir, int Ysen){
    ...
    if(int(senX_dir) == int(Xdir)){
        ...
        if(debug){
            ez::screen_print("X: true     Raw: " + util::to_string_with_precision(DSR::sensors[Xsen].read(Ydir)),5);
        }
    }else{
        chassis.odom_x_set(feild_size - DSR::sensors[Xsen].read(Ydir));
        if(debug){
            ez::screen_print("X: false     Raw: " + util::to_string_with_precision(DSR::sensors[Xsen].read(Ydir)),5);
        }
    }
    ...
    if(int(senY_dir) == int(Ydir)){
        ...
        if(debug){
            ez::screen_print("Y: true     Raw: " + util::to_string_with_precision(DSR::sensors[Ysen].read(Ydir)), 6);
        }
    }else{
        chassis.odom_y_set(feild_size - DSR::sensors[Ysen].read(Ydir));
        if(debug){
            ez::screen_print("Y: false     Raw: " + util::to_string_with_precision(DSR::sensors[Ysen].read(Ydir)), 6);
        }
    }
}
